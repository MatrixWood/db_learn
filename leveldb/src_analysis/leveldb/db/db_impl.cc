...
// 写任务合并是由函数BuildBatchGroup来完成的，BuildBatchGroup会把任务队列writers_中
// 若干个writer的数据合并到一起，从而减少磁盘写次数，提高写性能，虽然每次写入
// 都是往memtable里写，但是为了在故障发生时保障数据的完整性，每次写入都需要先预先
// 写日志（WAL）并落盘，所以每次写入都要写一次磁盘。

// 如果调用方是串行写入key-value的，那么不会有合并操作，只有当调用方是并发写入，
// 才会出现任务合并（只有这种情况，任务队列才可能同时存在多个任务）。每次执行
// BuildBatchGroup时，都会尝试把队列中还没有完成的任务进行合并。

// BuildBatchGroup把任务队列writers_中的Writer合并，并返回合并后的WriteBatch。
// 如果没有进行合并，会返回第一个Writer的batch，如果有合并就返回tmp_batch_，
// 合并结果放在tmp_batch_里面。
...
WriteBatch* DBImpl::BuildBatchGroup(Writer** last_writer) {
  ...
  // 实现的一个断言函数，
  // 这个是给编译器看的，如果编译器发现执行这里之前没有加锁，就会报错。
  mutex_.AssertHeld();
  ...
  // max_size会根据第一个Writer的size来调整，如果size比较小，
  // 就把max_size调低为size + (128<<10)，作者解释这里是为了耗时考虑，
  // 避免小size的写操作被延迟太久。
  ...
  *last_writer = first;
  ...
  for (; iter != writers_.end(); ++iter) {
    ...
    if (w->batch != nullptr) {
      ...
      // 这段逻辑只要是writers_中有任意一个(不包含第一个）可
      // 以合并的Writer就会执行且只执行一次，反之不会执行
      // Append to *result
